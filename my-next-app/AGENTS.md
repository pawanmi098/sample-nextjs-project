...
| Document upload — "Upload Documents" | `/document-upload` | `rxtwECShBU2TwVlnjvXQvI` (IVS) | `761:196637` "Document upload" 1280×1697 | `1168:23394` "Document Upload" 360×2816 (row states `1338:128141`) | `common/SiteHeader`, `common/PageIntro`, `common/BottomStickyBar` (`href`), `traveller-details/StepProgress` (`completed`), `traveller-details/SummaryCard`, `document-upload/DocumentChecklist`, `document-upload/TravellerDocuments`, `document-upload/DocumentGuide`, `document-upload/ApplicationIdCard`, `document-upload/CopyReference`, `document-upload/NextSteps`, `document-upload/PaymentSummary`, `document-upload/HelpCard`, `document-upload/MoreInfo` | Web + mweb implemented, uploads wired (row states web `1338:114192`, mweb `1338:128141`); tablet uses the mweb layout |
...
- mweb only: the plan card's "Validity" pill and "· 30 days duration", derived from the plan's `validity`/`stay` facts via `plans.summary`, plus the "For more information on the plan / View Details" block and the featured card's processing-time foot strip. The card's DOM is shaped for web, and below desktop `.head`/`.headRow`/`.body` are `display: contents` inside a one-column grid (see `VisaPlanCard.module.scss`). web only: the facts columns, the "Per Infant" price column and the sort button.
- The plan cards follow a newer web drawing than the rest of the page: `1075:305135` holds featured `1075:305136` (1192×190) and plain `1075:305174` / `1075:305210` (1192×178), superseding `724:142607` / `724:142645` inside the SRP frame. It adds the third price column and switches off both the View Details block and the foot strip, which is why those are mweb-only. On web the processing time reads from the facts row instead. The mweb frames are unchanged, so the card keeps two price columns and both blocks below desktop.
- The mweb chip row scrolls sideways and bleeds through the right gutter to the screen edge.
- The header's edit button opens `TripSearchPanel`, which is drawn twice and so behaves twice. Below desktop it's the full-screen "edit overlay" (`999:263879`): the `#F5F8FC` "Overlay" covers the header too, so the panel brings its own `<PopupModalHeader>` close button (32 from the top) and puts the fields in the 320-wide r12 card 35 below it; focus moves to that close button. At desktop it's the web frame's panel dropping under a header that stays visible, over the blue screen, and focus stays on the header's chevron. Same DOM either way — only the stylesheet switches.
  - That card runs one size down from the home mweb search card the shared `SearchForm` otherwise takes its mobile values from: 8/12 field padding, a 20/24 city, 14/20 values, 16 chevrons and a 40-tall 14/20 "Continue". Those live under `.panel` in `SearchForm.module.scss` and hand back to the shared values from desktop up, alongside the `search-form-*-compact` mixins. `editSearch.submitLabel` is `{ mobile, desktop }` for the same reason — "Continue" against the web frame's "Search".
  - Not built: the 16 chevron Figma draws after "Dubai" in the overlay's "Where" field (`999:264302`). It's absent from both the home mweb card and the web frame, so adding it means changing the shared destination field's DOM for one frame.
...
Notes for the document upload page (web `761:196637` "Document upload" 1280×1697, mweb `1168:23394` "Document Upload" 360×2816):
- The step after payment. The booking, the visa card and the price rows come from `travellerDetailsContent` (`booking`, `visaSummary`, and `resolveReviewApplication(bookingDigest(booking))`), so nothing about the booking is duplicated; the page's own copy is `documentUploadContent.json`.
- Every y in the web frame matches within 1px (columns 258, cards 302/968/1058, application ID card 1156, stepper 258, payment 863, help 1263, more information 1464); the page is 1698 tall against 1697.
- Shared additions: `StepProgress` takes `content.completed` for green finished bars (`--indicator-step-done`); `BottomStickyBar` takes `href` to render its button as an unprefetched link; `PageIntro`'s desktop subtitle line height is `--page-intro-subtitle-leading` (23 by default, this page sets 20, as the frame types it 16/20).
- **Uploading** (web `1338:114192` "Accordian" 803×652 and mweb `1338:128141` "Accordian" 320×560, each drawing the row states at once): each "Upload" is a `<label>` over a visually hidden `<input type="file">` (native picker, keyboard included), named `documents[<traveller>][<document>]`, and choosing a file uploads it straight away — there is no submit step. A row goes `idle → uploading → uploaded | failed`, and back to `idle` when cancelled:
  - **uploading** — "Uploading… {percent}%" over an 8-tall pill bar (`--bg-progress-track`/`--bg-progress-value`), with the navy cancel disc. Web: 98 tall (13 block padding) against the others' 96, the 44 disc centred against the text and bar together. mweb: 80 tall, a 24 disc on the title's line, and the bar runs from the text on past the disc (218 of the 272 content). The icon disc spans both grid rows, so the bar hangs 8 under the *text* and not under the taller of the two — that is what makes the mweb row 80 rather than 82.
  - **uploaded** — the file name in 500 (14/20 web, 12/18 mweb), and the green tick — 44 centred on web, 24 on the title's line on mweb. It is not a control: no frame takes an upload back.
  - **failed** — the row's edge goes `--border-error` and the message takes `--text-error` (400 14/20 on web, 500 12/18 on mweb — the one line the frames weight differently). "Try again" replaces "Upload": 128×40 beside the text on web, full width under it on mweb, which puts that row at 118. It resends the same file when the upload failed, and opens the picker instead when the file itself was refused (too large, wrong type), because resending it would fail the same way; that variant is a `<label>`, the other a `<button>`.
  - `src/lib/documentUpload.js` holds the states, the file check and the request. It posts through XMLHttpRequest, not fetch, because only XHR reports how much of the body has gone out. `POST /api/documents/upload` is a PROVISIONAL stand-in: it validates (7MB, jpeg/png/pdf) and acknowledges without storing. It answers failures with a `code`, never a sentence, so every string a traveller reads stays in `documents.status.errors`; the endpoint and the limit are JSON too (`documents.upload.endpoint`, `.maxBytes`).
  - The card owns its five rows' state, so `document-upload/TravellerDocuments` is a client island and `DocumentChecklist` stays a server section around it. "1 of 5 uploaded" counts the rows that got through. Each row carries a live region that is empty until an upload settles, so the outcome is announced without the region being born with its message; the bar is a `role="progressbar"` with `aria-valuenow`. The accordion is a controlled `<details>` now that its head re-renders.
  - Card, head and rows measure exactly as both frames draw them — web 803×652, head 74, rows at 90/202/314/428/540, heights 96/96/98/96/96; mweb 320×560, head 76, rows at 88/170/252/348/430, heights 66/66/80/66/118, the bar 218. Deliberate deviations: the web uploading and failed rows are drawn 4 further in (24 side padding) and aren't, since sibling rows would step sideways as an upload starts; the web progress bar reaches the cancel disc, where Figma stops it at 506 because the text frame beside it is a fixed width with 109 of slack; and the uploading line stays "Uploading... {percent}%" rather than mweb's "Uploading....60%". The mweb frame also types the photo row "Passport Photo" where the page frame (`1207:85331`) types it "Photograph", which is what the JSON keeps, and draws the flight ticket's icon on "Passport front page" — the same slip the page frame makes.
  - The `761:196473` "Re-upload the document for Nishi" error toast still has no caller: a failed row says it inline.
- The payment summary and the "More Information" rows are native `<details>`, so they work without JavaScript. The traveller cards are too, but they carry their uploads' state, so they hydrate; the page's other client islands are `CopyReference` (Booking ID and Transaction ID "Copy") and `DocumentGuide`.
- The document rows lay their 20 padding out from *inside* their 1 stroke (content at 21,21 in a 96 row), like the review strip; the traveller cards' heads don't.
- **mweb** (subtract 94 from its outline y-values): one column — pill, 16, stepper (`--page-intro-slot-margin`, new on `PageIntro`), 24, heading, 12, traveller cards; then visa summary, application ID, next steps, payment, help, more information, all 24 apart, over the 72-tall `review` sticky bar. The mweb frame puts the visa summary *before* the application ID card, so below desktop `.main`/`.aside` are `display: contents` in one flex column and `order: 1` moves the ID card and everything after the visa summary behind it (page.module.scss). Tab order still follows the DOM there.
- Every mweb y matches within 2–3, the 2 being the shared pill (27 tall against the frame's 25), except below the application ID card: our booking ID is 22 digits and wraps at 360, where the mweb frame's placeholder is 14 digits and fits, so everything after it sits 16 lower.
- mweb-only styling, each the base with the web values at `>=desktop`: no "Required Documents" title (visually hidden, kept for screen readers); a 20 #25304B chevron (`country-chevron-down.svg`, reused) where web has the 24 #000099 one; document rows stacking a full-width 36-tall "Upload" (1 edge, 500 12/18, 20 icon) under the text, with a 16 info icon; the copy controls as the icon alone (the word shows only while it says "Copied"); a lighter "Next Steps" (1 #E2EBF2 left rail, a 16 tick, plain #7A85A0 / #9BA4B8 dots, 12/18 descriptions); no chevron by "Payment Summary"; a right-pointing chevron on "Terms & Conditions"; 12 padding and 12 title gaps throughout. `SummaryCard`'s base styles now follow this frame too (12 gap, 12 padding) — traveller details hides it below desktop, so nothing else changed.
- Copy the frames word differently lives as `{ mobile, desktop }` and renders through `ResponsiveCopy`, now exported from `common/PageIntro` and also used by `BottomStickyBar` for its label: the heading and subtitle, "Photograph"/"Passport photo", "PAN card"/"PAN Card", "Need help ?", and "Track Application Status"/"Track your application".
- The documents follow the mweb order at both widths (photo, flight ticket, passport front, passport back, PAN card); mweb confirms the fifth web row is "Passport back page".
- Deliberate deviations: the "Powered by Calleo" pill keeps the shared `#565564` style rather than these frames' `#06066A` (400 12/18, 25 tall on mweb); mweb's "Passport front page" row shows the flight ticket's icon, which reads as a slip, so it keeps the profile icon; mweb's help card reads "Contact Calleo" and "123456786" where web reads "Contact Mokobara" and "7975298840" — contact details can't differ by width, so the web's are kept until someone confirms which is right.
- PROVISIONAL: the info icons' tip copy (`documents.required[].hint`) and presentation — Figma draws the icon only; the "Copied" feedback; the "Terms & Conditions" row's open content and its `/terms` link; the payment summary's closed state (its chevron turns to point right; on mweb, with no chevron, the title still toggles it). `/track-application` and `/terms` aren't routes yet.

- **Photo guide popup** (web `557:23258` "Photograph" 1280×785): the "Passport photo" row's info icon opens `document-upload/DocumentGuide`, a native `<dialog>` (`showModal()`, so inert page, focus kept inside, Escape) centred over the popup scrim at 207.41deg for this frame, with a 5.6 backdrop blur. It mounts only while open, so the sample photo loads on demand, and focus goes back to the icon on close. A document opts in with a `guide` in the JSON; the shared chrome is `documents.guide`. The others keep their PROVISIONAL tip. Every y matches within 1px. Deliberate deviation: "DO’S"/"DONT’S" take `--text-body`, not Figma's `#7A85A0` (3.7:1). Copy is as typed, including "Photograph Size : 35 x 45 MM" beside two "45 MM" arrows — worth confirming. PROVISIONAL: no mweb frame, so below desktop the card is full width less 16 gutters, scrolls inside the viewport and stacks the photo over the lists.
