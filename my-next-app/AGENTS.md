...
| Document upload — "Upload Documents" | `/document-upload` | `rxtwECShBU2TwVlnjvXQvI` (IVS) | `761:196637` "Document upload" 1280×1697 | `1168:23394` "Document Upload" 360×2816 | `common/SiteHeader`, `common/PageIntro`, `common/BottomStickyBar` (`href`), `traveller-details/StepProgress` (`completed`), `traveller-details/SummaryCard`, `document-upload/DocumentChecklist`, `document-upload/DocumentGuide`, `document-upload/ApplicationIdCard`, `document-upload/CopyReference`, `document-upload/NextSteps`, `document-upload/PaymentSummary`, `document-upload/HelpCard`, `document-upload/MoreInfo` | Web + mweb implemented (first view only); tablet uses the mweb layout |
...
- Only the first view is built. Each "Upload" is a `<label>` over a visually hidden `<input type="file">` (native picker, keyboard included), named `documents[<traveller>][<document>]`, but nothing is posted or shown after a file is chosen yet — progress, the uploaded state and the "Re-upload the document for Nishi" error toast (`761:196473`, already built as `common/Toast`) wait for their frames. Counts are hard-wired to 0.
- The traveller cards, the payment summary and the "More Information" rows are native `<details>`, so they work without JavaScript. The one client island is `CopyReference` (Booking ID and Transaction ID "Copy").
